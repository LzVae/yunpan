...
public class LogUtil {
	 ...
	 * </p>
	 */
	public void writeDeleteFileEvent(HttpServletRequest request, com.zxd.pan.model.File f) {
		if (ConfigureReader.instance(request).inspectLogLevel(LogLevel.Event)) {
			String account = (String) request.getSession().getAttribute("ACCOUNT");
			if (account == null || account.length() == 0) {
				account = "Anonymous";
	 ...
	 * </p>
	 */
	public void writeUploadFileEvent(HttpServletRequest request, com.zxd.pan.model.File f) {
		if (ConfigureReader.instance(request).inspectLogLevel(LogLevel.Event)) {
			String account = (String) request.getSession().getAttribute("ACCOUNT");
			if (account == null || account.length() == 0) {
				account = "Anonymous";
	 ...
	 * </p>
	 */
	public void writeDownloadFileEvent(HttpServletRequest request, com.zxd.pan.model.File f) {
		if (ConfigureReader.instance(request).inspectLogLevel(LogLevel.Event)) {
			String account = (String) request.getSession().getAttribute("ACCOUNT");
			if (account == null || account.length() == 0) {
				account = "Anonymous";
	 ...
	 * </p>
	 */
	public void writeRenameFileEvent(HttpServletRequest request, com.zxd.pan.model.File f, String newName) {
		if (ConfigureReader.instance(request).inspectLogLevel(LogLevel.Event)) {
			String account = (String) request.getSession().getAttribute("ACCOUNT");
			if (account == null || account.length() == 0) {
				account = "Anonymous";
		...
		if (ConfigureReader.instance(request).inspectLogLevel(LogLevel.Event)) {
			...
			Thread t = new Thread(() -> {
				...
				for (String fid : idList) {
					com.zxd.pan.model.File f = fim.queryById(fid);
					if (f != null) {
						Folder folder = fm.queryById(f.getFileParentFolder());
						List<Folder> l = fu.getParentList(folder.getFolderId());
						String pl = new String();
